...
// include contracts-info even for DEBUG (test) builds to ensure it generated well
...
// int contractsLength = sizeof(contracts) / sizeof(contracts[0]);

contract_info_t *find_contract_info(const char *a) {
    int i = sizeof(contracts) / sizeof(contracts[0]);
    while (i--) {
        if (eq(a, contracts[i].address))
            return &contracts[i];
    }
